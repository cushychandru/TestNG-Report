...
public class Listener implements ITestListener, ISuiteListener, IInvokedMethodListener {
	...
	ArrayList<String>		faildClasses	= new ArrayList<String>();
...
try{
		if (status.equals("FAILED") && result.getThrowable().getMessage() != null) {
			...
			if (excepetion.contains("Command duration or timeout")) {
				excepetion = excepetion.substring(0, excepetion.indexOf("Command duration or timeout"));
			}
			...
			for (int i = 0; i < excepetion2.length; i++) {
				if (excepetion2[i].getClassName().equalsIgnoreCase(_class)) {
					excepetion += "<br> Class Name :  " + _class + "<br> File Name :  " + excepetion2[i].getFileName() + "<br> Error Line Number :  " + excepetion2[i].getLineNumber();
				}
			}
			...
			if (faildClasses.contains(_class) == true) {
				
					XmlWriter.FailedSuitWriter(_class);
				
				faildClasses.add(_class);
			}
		}else if(status.equals("FAILED")||status.equals("SKIPED")) {
			XmlWriter.FailedSuitWriter(_class);
			faildClasses.add(_class);
		}
}catch (Exception e) {
	...
	faildClasses.add(_class);
		...
		try {
			...
			XmlWriter.JSON_report(new Gson().toJson(Report.report));
